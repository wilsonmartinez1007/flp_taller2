  ...
  (lambda (prim lval)
    (cases primitiva prim
      ...
      (length-prim ()
                   (if (list? (car lval))
                       (length (car lval))
                       (eopl:error "Argumento de length no es una lista")))
      ...
      (rest-prim ()
                 (if (list? (car lval))
                     (if (null? (car lval))
                         ...
                         (cdr (car lval)))
                     (eopl:error "Argumento de rest no es una lista")))
      ;; N-ésimo elemento de la lista
      (first-prim ()
                  (if (list? (car lval))
                      (if (null? (car lval))
                          (eopl:error "La lista está vacía")
                          (car (car lval)))
                      (eopl:error "Argumento de first no es una lista")))
      (nth-prim ()
                (let ((lst (car lval))
                      (n (cadr lval)))
                  (if (list? lst)
                      (if (and (integer? n) (>= n 0))
                          (list-ref lst n)
                          (eopl:error "Índice inválido para nth"))
                      (eopl:error "Argumento de nth no es una lista"))))
